...
from graphify.platform_qwen import (
    QWEN_HOOK as _QWEN_HOOK,
    QWEN_MD_MARKER as _QWEN_MD_MARKER,
    install_qwen_skill as _install_qwen_skill,
    qwen_install,
    qwen_uninstall,
)
...
def install(platform: str = "claude") -> None:
    ...
    if platform == "qwen":
        ...
        print()
        print("Done. Open Qwen Code and invoke the graphify skill.")
        print()
        return
...
def run_pipeline(path: Path = Path("."), *, no_viz: bool = False) -> None:
    ...
    if not code_files:
        print(f"No code files found in {root}; full semantic pipeline is not available from the CLI run command.", file=sys.stderr)
        sys.exit(1)

    print(f"Step 2: extract AST from {len(code_files)} code files")
    ...
    print("DONE")
    print(f"graph_report {(out / 'GRAPH_REPORT.md').exists()}")
    print(f"graph_json {(out / 'graph.json').exists()}")
    print(f"graph_html {(out / 'graph.html').exists() if html_written else False}")
...
def main() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help"):
        ...
        print("  run <path>             run the full local graphify pipeline and export graph outputs")
        ...
        print("  qwen install           configure Qwen Code (.qwen/skills + QWEN.md + PreToolUse hook + MCP)")
        print("  qwen uninstall         remove Qwen Code graphify configuration")
        print()
        return
    ...
    if cmd == "install":
        ...
        if not run_path.exists():
            ...
            sys.exit(1)
        run_pipeline(run_path, no_viz=no_viz)
    elif cmd == "claude":
        ...
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            qwen_install(Path("."), Path(__file__).parent)
        elif subcmd == "uninstall":
            qwen_uninstall(Path("."))
        else:
