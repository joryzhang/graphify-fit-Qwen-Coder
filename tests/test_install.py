...
def test_install_qwen(tmp_path):
    _install(tmp_path, "qwen")
    assert (tmp_path / ".qwen" / "skills" / "graphify" / "SKILL.md").exists()
...
def test_qwen_skill_mentions_agent_tool_and_todo():
    import graphify
    skill = (Path(graphify.__file__).parent / "skill-qwen.md").read_text()
    assert "Agent" in skill
    assert "todo_write" in skill
