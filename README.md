...
## Qwen Code Enhancements in This Fork

This fork includes additional Qwen Code-focused work beyond the upstream graphify project:

- `graphify qwen install` / `graphify qwen uninstall`
- a dedicated `graphify run <path>` full pipeline CLI entrypoint
- a repository-local Qwen Code extension prototype under [`qwen-extension/`](./qwen-extension)

If you want the command-style Qwen experience, start with the extension prototype in `qwen-extension/`.

### Local Qwen extension usage
...
Then restart Qwen Code and try:
...
> Note: the extension currently targets preview / experimental usage and expects an installed `graphify` CLI that supports `graphify run <path>`.
...
**Qwen Code** writes a project skill to `.qwen/skills/graphify/SKILL.md`, appends a `QWEN.md` section so graph context is always available in project memory, installs a `PreToolUse` hook in `.qwen/settings.json` for `read_file`, `glob`, `grep_search`, and `list_directory`, and registers a `graphify` MCP server in `.qwen/settings.json`. This makes Qwen Code prefer graph outputs over broad raw-file searching when a graph exists.
